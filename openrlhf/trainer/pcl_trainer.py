...
class PCLTrainer(ABC):
    ...
    def fit(self, args):
        ...
        for epoch in range(self.epochs):
            ...
            neg_log_weights = []
            # train
            for ids, masks, state_masks, action_masks, rewards, length in self.train_dataloader:
                if self.packing_samples:
                    ids = ids.to(torch.cuda.current_device())
                    masks = masks.to(torch.cuda.current_device())
                    state_masks = state_masks.to(torch.cuda.current_device())
                    action_masks = action_masks.to(torch.cuda.current_device())
                ...
                with torch.no_grad():
                    ...
                    accumulated_logps, logps, _ = self.accumulated_logps(model, ids, masks, action_masks, length)
                
                values: torch.Tensor = self.critic(ids, action_mask=action_masks, attention_mask=masks, ring_attn_group=self.strategy.ring_attn_group, packed_seq_lens=length)
                ...
                if train_actor:
                    accumulated_logps, logps, logps_raw = self.accumulated_logps(self.model, ids, masks, action_masks, length)
                    # with torch.no_grad():
    ...
    def accumulated_logps(self, model, ids: torch.Tensor, masks: torch.Tensor, action_masks: torch.Tensor, length: torch.Tensor):
