...
def _get_critic_model(base_pretrained_model, base_llm_model, packing_samples=False):
    class LLMForSequenceRegression(base_pretrained_model):
        ...
        def forward(
            ...
            if ring_attn_group is not None:
                last_hidden_states = all_gather(last_hidden_states, ring_attn_group)
                last_hidden_states = last_hidden_states.reshape(outputs["last_hidden_state"].shape[0], total_seq_len, -1)

            values = self.value_head(last_hidden_states).squeeze(-1)[:, :-1]

            # num_actions = []
            # for action_mask_sig in action_mask:
            #     num_actions.append(action_mask_sig.size(0))
            ...
            #         start, end = max(0, offset + seq_len - num_action), offset + seq_len
